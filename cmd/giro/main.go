...
func runDemo() error {
	...
	return generatePaymentDocument(payment, "payment.html", time.Now())
}
...
func createCommand(args []string, outputPath string) error {
	...
	if err := generatePaymentDocument(payment, outputPath, time.Now()); err != nil {
		return err
	}
...
func generatePaymentDocument(payment epc.Payment, outputPath string, generatedAt time.Time) error {
	...
	html, err := document.Render(payment, generatedAt, png)
	if err != nil {
